...
\begin{enumerate}
    \item Verify if speeding behavior in Curitiba is spatially autocorrelated. If so, identify possible causes for this behavior;
...
This chapter contains the discussion based on the results presented in the last chapter. The section disposition is related to the specific objectives listed in \autoref{sec:obj}:

\begin{enumerate}
    \item Verify if speeding behavior in Curitiba is spatially autocorrelated. If so, identify possible causes for this behavior - \autoref{sec:sp_corr};
    \item Verify if the GWR model can properly analyze the influence of the BE in the occurrence of speeding in a spatially autocorrelated scenario - \autoref{sec:gwr_perf};
    \item Identify evidences of the influence of the BE in the occurrence of speeding in Curitiba - \autoref{sec:be_sp};
    \item Identify practical solutions to mitigate speeding behavior through urban planning practices, based on the results - \autoref{sec:sp_manag}.
\end{enumerate}
