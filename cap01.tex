...
In 2010, the United Nations (UN) declared the Decade of Action for Road Safety 2011-2020, calling the signatory countries to reduce in half the predicted number of deaths and injuries in traffic until 2020 \cite{WHO2011}. In Brazil, the National Plan of Reduction of Deaths and Injuries (PNATRANS) was created in 2018, aiming to reduce the national fatality rate (deaths per 10,000 vehicles) and mortality rate (deaths per 100,000 population) in half, within the period of 2019-2028 \cite{MinistryofCities2018}. Following the Decade of Action 2011-2020, the UN declare another Decade of Action for Road Safety with the same objectives of the last one, but considering a time frame of 2021-2030 \cite{WHO2020}.

One of the main risk factors of road crashes is speeding, which increases the severity and occurence of crashes \cite{Mohan2016a}. In urban areas, the severity of crashes has a higher impact to more vulnerable users, like pedestrians and cyclists \cite{Welle2016}. The characteristics inside the urban environment, including land use codes and the built environment can mitigate the occurence of speeding and improve the road safety performance \cite{Knoflacher2016}. Therefore, this work aims to better explore the relationship between the built environment inside cities and the level of road safety performance, considering the occurence of speeding as a intermediate outcome. 

\section{OBJECTIVES}

In the context of urban planning and road safety management, the main objective of this research is to investigate the influence of demographic, land use and constructed environment on the speeding practices as a risk factor on the frequency and severity of road crashes. The scenario of the study is the city of Curitiba, capital of the state of Paraná, Brazil. The correlation will be analyzed with the use of the Geographically Weighted Regression statistical model.
...
The excess of traffic speed performed by vehicles on urban environments are a main risk factor in the chance and severity of road crashes, considering there is a great volume of interaction between motorized and vulnerable users \cite{Elvik2009}. Considering the direct relationship between the mobility, the land use patterns and the built environment \cite{DeVos2013}, it is important to investigate these factors in search of improvements in the management process of the road safety. Speeding, which is on the primary causes for traffic crashes \cite{WHO2013} can be better studied and analyzed by the collection of naturalistic data.

Having in mind that most of the traffic crashes and conflicts happens in cities \cite{WHO2018}, as a consequence of the fast process of urbanization in the last decades, it is crucial to create safer conditions in these localities. In Brazil, the management of the mobility is attributed to the municipalities \cite{Brasil1997}. Therefore, the management of road safety, as a task inherent to traffic management, is also an attribution of the municipalities. It is necessary to observe the issues of road safety in cities when conducting the process of urban planning. Some national laws foresees this integration between the planning process by the municipalities and the sustainable mobility are clearly defined: The Statute of the Cities \cite{Brasil2001} and The Law of the Urban Mobility \cite{Brasil2012}.    
...
This document is divided into five chapters. Next to this chapter, Chapter \ref{cap:lr} includes the literature review, Chapter \ref{cap:methods} contains the methodological proceedings used in this work and Chapter \ref{cap:results} includes the data that resulted from the method and it's analysis. Chapter \ref{cap:conclusion} includes a brief conclusion of the thesis. 
...
\section{NATURALISTIC DRIVING STUDY - BRAZIL} \label{ndsc}
...
This chapter focus on the results from the NDS-BR (distances, speeding) and the results from the GWR (correlation between speeding and built environment). 
...
All GWR models presents a better performance in comparison to the global linear model, indicating that GWR is a better method to explore the relationship between speeding and the built environment. When applying Moran's $I$ on the speeding variable, it resulted in a value of 0.318 ($p$-value of 0.001), revealing that speeding is spatially autocorrelated in a clustered aspect. \autoref{tab:lm_results} shows the coefficient estimates of the linear regression model, in addition to the standard error, $t$-values and $p$-values. The only coefficient statistical significant at the 95\% level is the density of speed cameras (DSC), which express a inverted correlation to speeding.
...
\autoref{tab:gwr_results} contains the descriptive statistics of the chosen GWR model coefficient estimates. Maps in FIGURES \ref{fig:gwr_dsc_par} to \ref{fig:gwr_ldi_bsd} shows these coefficients in each TAZ. \autoref{fig:r2_map} shows the variation of local geographically weighted $R^2$. The western part of the city displays TAZs with higher values of $R^2$, contrary to the east side of Curitiba, which displays lower values. Local $R^2$ values varied between 0.221 and 0.825. 
...
\autoref{fig:sp_mean_sd} displays the geographically weighted local mean and local standard deviation of the speeding variable across Curitiba. The western ou southern parts of the city presented a higher values, while the central area presented lowe values of speeding. The east side of Curitiba also has higher values of speeding, indicating a pattern where more peripheral TAZs has higher values of speeding than central areas. Regarding the standard deviation (SD) of the speeding data, the western side of the city has TAZs with higher values, while the southeastern side has lower values. SD had a variation between 0.105 and 0.171.  
...
\autoref{fig:gwr_dsc_par} displays the coefficient estimates of the density of speed cameras (DSC) and proportion of arterial roads (PAR). DSC presented a inverted correlation to speeding in most of the TAZs. PAR had mixed results, presenting direct and inverted correlations to speeding in different areas of the city. 
...
The density of intersection (DIS) and density of commercial and services units coefficient estimates are displayed in \autoref{fig:gwr_dis_dcsu}. DIS has a inverted correlation to speeding in the northeast part of the city, while the west and south sides shows TAZs with a direct correlation between speeding and DIS. Regarding DCSU, almost all of the TAZs shows a inverted correlation to the dependent variable.  
...
\autoref{fig:gwr_avi_pd} displays the coefficient estimates of average income (AVI) and population density (PD). AVI coefficients showed a inverted correlation to speeding in TAZs closer to the north and center of the city, while the southern side contains TAZs with a direct correlation between AVI and speeding. PD is directly correlated to speeding in most of the area, with higher values in the southern part of the city. 
...
Land use diversity index (LDI) and bus stop density (BSD) coefficient estimates are plotted in \autoref{fig:gwr_ldi_bsd}. LDI coefficients shows a direct correlation to speeding in most of the TAZs, with higher values in the northeast part of Curitiba. BSD shows mixed results, with similar quantity of TAZs with direct and inverted correlation to speeding.  
...
Finally, \autoref{tab:coef_perc} presents a list with the percentage of TAZs with positive and negative coefficients, for each variable presented in the maps. 
...
%% [x] DSC is inverted correlated to speeding in 97.2% of the current sample of TAZs.
...
The Naturalistic Driving Study enabled the collection of speed data from a sample of 16 drivers in the city of Curitiba, Brazil. Comparing to the described NDS in Section \ref{nds}, 16 volunteers is a small sample to conduct a more significant analysis. Some volunteers had more participation in the sample than others. Drivers $D_{14}$ and $D_{15}$ had the highest proportion in the total travelled distance:  19\% and 23\%, respectively. Regarding the spatial coverage, most of the travelled distance of the sample was performed in central areas of the city, in comparison to peripheral areas. The quantity of travelled distance wasn't enough to cover all the present TAZs in Curitiba (reducing 135 into 107).

In addition to the built environment, other attributes can influence resultant speeding values. Characteristics from the drivers (age, experience, license age, education) and from the vehicles (age, engine power) also can influence the speeding results. Using \textcite{Richard2013a} method of speeding behavior classification, all drivers from the current sample were classified as "habitual speeders". The study would be more complete if the sample contained drivers with other speeding behaviors as well. 

The Geographically Weighted Regression with a Boxcar kernel had the best performance, comparing to other kernel types and the linear regression. But none of the models presented a satisfactory performance, considering that the residuals form all models are spatially autocorrelated. This indicates that they couldn't validly predict the spatial heterogeneity present in the occurence of speeding. All GWR models showed a better performance than the linear regression. To \textcite{Brunsdon2010}, calibration of the GWR model for regions close to the edge of the study area are subject to greater sampling error. The "holes" present in the current sample of TAZs could have caused a negative impact on the GWR performance. 

Based on the results from the linear regression (\autoref{tab:lm_results}), only the density of speed cameras (DSC) coefficient showed statistical significance. It's inverted correlation to speeding is similar to previous results found by \textcite{Li2020,Li2013a,Oliveira2015}. The GWR model indicates a inverted correlation between DSC and speeding on 97.2\% of the current sample of TAZs (\autoref{fig:gwr_dsc_par}, \autoref{tab:coef_perc}). It is important to remind that this correlation does not imply causation, therefore further investigation is needed. 

Before reaching to any further conclusions, it is important to perform new models based on a sample with a higher number of drivers. NDS-BR is still ongoing, making able to collect more speeding data from new volunteers. More testing with the independent variables is also needed, in order to identify key variables that works better within the model. 