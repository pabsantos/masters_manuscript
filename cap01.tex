...
% \autoref{tab:drivers} contains information about the participants and their vehicles, including the duration of data collection, age, sex, age of driver's license, vehicle model year, and horsepower.
...
%% Geographical data are scale dependent.
%% There are multiple ways to divide geographical data into different zonal systems
%% Each zonal system will provide different results for the same geographical data.
%% This problem is known as the modifiable areal unit problem (MAUP).
%% Aggregated data will always present this problem.
%% The zoning effect: same data with different zoning boundaries will produce inconsistent results.
%% Ecological fallacy problem: inaccuracies of inferring individual situation based upon aggregated data.
...
The final step was to analyze the performance between models. This analysis was based on four performance indicators: $AIC_c$, $R^2$, adjusted $R^2$, and the value of Moran's $I$ on residuals. Higher values of $R^2$ and adjusted $R^2$ indicate a better performance.
...
Both maps in \autoref{fig:taz_sp} can serve as a reference for speeding mitigation measures, making it possible to identify the most serious locations. Speed limit enforcement and other speed control measures should focus on high speeding zones and High-High clusters. Also, the speed management practices must be integrated with other jurisdictions (state and/or federal), especially in areas where highways are present (\autoref{fig:road_cwb}). Urban planners and future planning policies need to know that transportation and road safety issues are spatially autocorrelated, and these issues are heavily influenced by the surrounding built environment and the established land use laws. Therefore, future data analysis regarding road safety should include the spatial dimension to improve its results. 
...
%% [x] a cidade nao deve aumentar a quantidade de arteriais, mas entender os fatores que levam à maior segurança dessas vias e aplicá-los em outras vias da cidade.
